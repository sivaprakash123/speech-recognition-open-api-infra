...
def append_config(command, cuda_visible_devices, enable_gpu, node_name, replica_count, gpu_count, cpu_count,
    ...
    if enable_gpu:
        gpu_command = "--set resources.limits.\"nvidia\.com/gpu\"='{}' --set env.gpu='{}'".format(gpu_count, enable_gpu)
...
class LanguageConfig:
    ...
    def deploy(self, namespace, api_changed, gpu_count, enable_gpu, node_selector_accelerator, cpu_count, image_name,
        ...
        command = append_config(command, cuda_visible_devices, enable_gpu, node_name, replica_count, gpu_count,
                                cpu_count, node_selector_accelerator)
        print('Running command: ', command)
...
class EnvoyConfig:
    ...
    def deploy(self, namespace, enable_ingress, enable_envoy_admin):
        ...
        command = "helm {0} --timeout 180s {1} {2} --namespace {3} --set ingress.enabled='{4}' --set envoyAdmin.enabled='{5}'".format(
            process,
            self.release_name,
            self.helm_chart_path,
            namespace,
            enable_ingress, enable_envoy_admin)
...
class ProxyConfig:
    ...
    def deploy(self, namespace):
        ...
        print('Running Proxy Command:', command)
