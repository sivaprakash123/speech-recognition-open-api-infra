...
class LanguageConfig:
    ...
    def deploy(self, namespace, api_changed, gpu_count, enable_gpu, node_selector_accelerator, cpu_count, image_name, image_version):
        ...
        command = "helm {0} --timeout 180s {1} {2} --namespace {3} --set env.languages='[\"{4}\"]' --set image.pullPolicy='{5}' --set image.repository='{6}' --set image.tag='{7}'".format(process, self.release_name, self.helm_chart_path, namespace, self.language_code, pull_policy, image_name, image_version)
        if enable_gpu == True:
...
class MultiLanguageConfig:
    ...
    def deploy(self, namespace, api_changed, gpu_count, enable_gpu, node_selector_accelerator, cpu_count, image_name, image_version):
        ...
        command = "helm {0} --timeout 180s {1} {2} --namespace {3} --set env.languages='[{4}]' --set image.pullPolicy='{5}' --set image.repository='{6}' --set image.tag='{7}'".format(process, self.release_name, self.helm_chart_path, namespace, languages, pull_policy, image_name, image_version)
        if enable_gpu == True:
            command = "{} {}".format(command, set_gpu_command)
        else:
            command = "{} {}".format(command, set_cpu_command)
